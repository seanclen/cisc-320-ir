...
int main() {
    ...
    vector<Node*> subtypes = getSubtypes("Cat", 1, INFINITY);
    for (Node* n : subtypes) {
        cout << n->data << endl;
    }
    
    // Get the number of subtypes
    cout << "\ngetNumSubtypes " << getNumSubtypes("Test") << endl;
    
    // Find the shortest path from a starting node and search for the species
    vector<Node*> path;
    cout << "getPath" << endl;
    getPath(path, graph, "HORSE");
    for (Node* n : path) {
        cout << n->data << endl;
    }
    
    // Find the lowest common ancestor between species
    Node* node = getLowestCommonAncestor(graph, "Dog", "Cat");
